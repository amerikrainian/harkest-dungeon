...
namespace DD2A11y.Core.Nav {
    /// <summary>
    /// Owns navigation: consumes semantic UI actions, holds the focus path (within the screen,
    /// excluding the root), and centralizes focus-path diffing + announcement. Speech is injected as
    /// a delegate so Core stays engine-free and the navigator is unit-testable.
    ///
    ...
    public abstract class Navigator {
        ...
        /// introspection; navigation still mutates the path only through the Navigator's own
        /// methods.</summary>
        ...
        public event Action<UIElement?>? FocusSettled;

        /// <summary>Bind to a screen root and set initial focus silently. The caller announces the
        ...
        /// focusable and return true so the caller can announce the new focus. Returns false when
        /// focus is still valid (the common case - a rebuild that did not touch the focused
        /// branch).</summary>
        ...
        // Focus is orphaned if the chain from the focused leaf up to the root is broken at ANY link,
        // not just the first: a rebuild can swap a whole nested subtree, leaving the leaf's immediate
        // parent still listing it while an ancestor higher up no longer does. Walk every link.
        ...
        /// <summary>Handle a semantic UI action (a <see cref="UiActions"/> key). Returns true if
        /// consumed.</summary>
        public abstract bool Handle(string actionKey);

        /// <summary>Move focus to a specific element (e.g. restoring position after a screen rebuilt
        /// its content under the cursor). Rebuilds the path to it; speaks the landing only when
        /// <paramref name="announce"/> is set. The landing supersedes current speech by default;
        /// pass <paramref name="interrupt"/> false to queue it when the move rides an action whose
        /// own feedback is still speaking.</summary>
        public void Focus(UIElement target, bool announce, bool interrupt = true) {
            var snapshot = new List<UIElement>(Path);
            BuildPathTo(target);
            if (announce) {
                AnnounceDelta(snapshot, interrupt);
            } else {
                SettleFocus();
            }
        }
        ...
        /// <summary>Announce the full focus path (screen entry): diff from empty, so container labels
        /// plus the focused leaf are read, e.g. "main menu, Continue, button".</summary>
        ...
        /// <summary>Rebuild the path as the ancestor chain from the root down to
        /// <paramref name="target"/>, setting each container's remembered focus along the way.</summary>
        ...
        protected void AnnounceDelta(List<UIElement> oldPath, bool interrupt) {
            int i = 0;
            while (i < oldPath.Count && i < Path.Count && oldPath[i] == Path[i]) {
                i++;
            }
        ...
        // pass through: let the element sync the platform cursor, then tell the buffer bridge.
        private void SettleFocus() {
            Current?.OnFocused();
            FocusSettled?.Invoke(Current);
        }

        /// <summary>Ordered Tab-stops: descend through Panels; a list is one stop (its representative
        /// item).</summary>

namespace DD2A11y.Core.Nav {
    /// <summary>
    /// The keys of the UI-category input actions the navigator consumes. The plugin registers
    /// <c>InputAction</c>s under these keys and routes a fired one to <see cref="Navigator.Handle"/>,
    /// which switches on them. Shared here so registration and dispatch agree on one set of names.
    /// </summary>

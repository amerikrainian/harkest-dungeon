...
namespace DD2A11y.Core.Strings {
    ...
    /// The plugin's language sync loads a file matching the game language and calls <see cref="Load"/>;
    ...
    public static class Translation {
        ...
        /// <summary>Whether a translation currently overrides this key.</summary>
        ...
        /// (typos, or a file from a newer mod), keys with empty values (skipped - an empty string
        /// would silence information), and a "_plural" naming no known rule (English kept).</summary>
            ...
            foreach (var entry in entries) {
                if (entry.Key == PluralKey) {
                    ...
                    if (rule != null) {
                        ...
                        report.UnknownPluralRule = entry.Value;
                    }
                    continue;
                }
                if (string.IsNullOrWhiteSpace(entry.Value)) {
                    report.EmptyKeys.Add(entry.Key);
                    continue;
                }
                if (!Strings.DefinesKey(entry.Key)) {
                    report.UnknownKeys.Add(entry.Key);
                    continue;
                }
        ...
        /// <summary>
        /// Parse a translation file: one "key = value" per line, splitting at the first '=' (values
        /// may contain '='), '#' lines are comments, blank lines skipped. Malformed lines land in
        ...
        public static Dictionary<string, string> ParseFile(string content, out List<string> errors) {
            ...
            for (int i = 0; i < lines.Length; i++) {
                ...
                int eq = line.IndexOf('=');
                if (eq <= 0) {
                    errors.Add("line " + (i + 1) + ": no 'key = value'");
                    continue;
                }

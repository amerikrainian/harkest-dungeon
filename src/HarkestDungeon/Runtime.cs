...
namespace DD2A11y {
    ...
    public sealed class Runtime : IDisposable {
        ...
        public Runtime(string pluginDir, string version, BepInEx.Configuration.ConfigFile config) {
            ...
            _roadSense = new Game.RoadSense(Audio, speak, Gate, Settings.SensingRange, Settings.AutoCollect);
            ...
            // taking surface. Its cursor moves re-home the buffers the same way a focus
            // change does.
            Router.Register(new MapScreen(speak, () => RehomeBuffers(Navigator.Current), Input));
            // The free-driving floor: the HUD as Tab panels around a pass-through driving
        ...
        private void RegisterInputs() {
            InputAction Reg(string key, Func<string> label, Action handler = null,
